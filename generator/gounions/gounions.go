...
func codeForUnion(u *an.Union) gen.Declaration {
	...
	// the JSON routines
	return gen.Declaration{ID: u.Name().Obj().Name(), Content: jsonForUnion(u)}
}
...
func jsonForUnion(u *an.Union) string {
	...
	for _, member := range u.Members {
		memberName := member.Name().Obj().Name()
...
func codeForNamed(typ *an.Named) (out []gen.Declaration) {
	...
	case *an.Array:
		if elem, isElemUnion := under.Elem.(*an.Union); isElemUnion {
			...
			out = append(out, gen.Declaration{ID: typ.Name().Obj().Name(), Content: jsonForArray(typ)})
		}
		return out
	case *an.Map:
...
func jsonForArray(typ *an.Named) string {
	...
	name := typ.Name().Obj().Name()
	elemName := elem.Name().Obj().Name()
...
func codeForStruct(st *an.Struct, cache gen.Cache) (out []gen.Declaration) {
	...
	for _, field := range st.Fields {
		...
		fieldType := types.TypeString(field.Field.Type(), myQualifier(st.Name().Obj().Pkg()))
